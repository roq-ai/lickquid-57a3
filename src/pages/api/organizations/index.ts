...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createOrganization() {
    ...
    const body = { ...req.body };
    if (body?.washing_machine?.length > 0) {
      const create_washing_machine = body.washing_machine;
      body.washing_machine = {
        create: create_washing_machine,
      };
    } else {
      delete body.washing_machine;
    }
