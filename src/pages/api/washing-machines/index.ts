...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getWashingMachines() {
    const data = await prisma.washing_machine
      .withAuthorization({
        roqUserId,
        tenantId: user.tenantId,
        roles: user.roles,
      })
      .findMany(convertQueryToPrismaUtil(req.query, 'washing_machine'));
    return res.status(200).json(data);
  }

  async function createWashingMachine() {
    ...
    const data = await prisma.washing_machine.create({
      data: body,
    });
    return res.status(200).json(data);
  }
