...
import { deleteWashingMachineById } from 'apiSdk/washing-machines';
  ...
  const { data, error, isLoading, mutate } = useSWR<OrganizationInterface>(
    ...
    () =>
      getOrganizationById(id, {
        relations: ['user', 'washing_machine'],
      }),
  );

  const washing_machineHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
        ...
        {isLoading ? (
          ...
          <>
            ...
            <Box>
              ...
              {hasAccess('washing_machine', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    <Text fontSize="lg" fontWeight="bold">
                      Washing Machines
                    </Text>
                    <NextLink passHref href={`/washing-machines/create?organization_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.washing_machine?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/washing-machines/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.size}</Td>
                            <Td>{record.type}</Td>
                            ...
                            <Td>
                              {hasAccess('washing_machine', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/washing-machines/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('washing_machine', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    washing_machineHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
